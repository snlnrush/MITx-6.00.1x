...
This pseudocode is provided to help guide you in writing your function. Check out the Why Pseudocode?
resource to learn more about the What and Why of Pseudocode before you start coding your solution.
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while True:
        ...
        displayHand(hand_temp)
        input_word = input('Enter word, or a "." to indicate that you are finished: ')
        assert not input_word.isdigit(), print('Digital in the word!!!')
        if input_word == '.':
            print('Goodbye! Total score: {} points.'.format(total_score))
            break
        if not isValidWord(input_word, hand_temp, wordList):
            print('Invalid word, please try again. \n')
            continue
        total_score += getWordScore(input_word, n)
        hand_temp = updateHand(hand_temp, input_word)
        print('"{}" earned {} points. Total: {} points \n'.format(input_word, getWordScore(input_word, n), total_score))
        if calculateHandlen(hand_temp) == 0:
